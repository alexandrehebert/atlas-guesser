...
function getCurvedConnectorPath(params: {
  ...
  // control point always curves upward (negative Y in screen space)
  const controlX = midX;
...
export default function GameMap({ onInitialZoomEnd }: { onInitialZoomEnd?: () => void }) {
  const t = useTranslations('guesser');
  const { quiz, round, answer, hoveredCode, accent, submitAnswer, setHoveredCode, mode } = useGame();
  const { isMobile, sidebarOpen, sidebarRef, sidebarToggleRef, topBarRef } = useGameLayout();
  const { svgRef, pathRefs, mapTransform, focusCountry } = useGameMap();
  ...
  // Zoom to the correct country after a wrong answer in map-based, country-to-name, and country-to-capital modes
  useEffect(() => {
    if (
      (mode === 'flag-to-country' || mode === 'capital-to-country' || mode === 'name-to-country' || mode === 'country-to-name' || mode === 'country-to-capital') &&
      answer &&
      !answer.correct &&
      round.targetCode
    ) {
    ...
    // Only run when answer changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [answer, mode, round.targetCode]);

  // Notify parent when initial zoom animation is done
  useEffect(() => {
    if (!onInitialZoomEnd) return;
    // Wait for a short delay to ensure zoom animation is finished
  ...
  const answerMarkers = useMemo(() => {
    ...
    // Convert actual UI element bounds to SVG coordinate space
    ...
    // Compute safe viewport: full visible area shrunk by edge padding and UI overlay bands.
    // viewportRect controls CLAMPING — labels are forced to stay inside it.
    ...
    if (topBarSvgRect) {
      // Full-width top band: push safe area below the bottom of the top bar
      safeTop = Math.max(safeTop, topBarSvgRect.y + topBarSvgRect.height + uiGapPx * unitsPerPxY);
    }
    if (sidebarSvgRect) {
      if (isMobile) {
        // Mobile: sidebar is a bottom sheet — push safe area above it
    ...
    // Build full-extent blocked strips for scoring penalty (belt-and-suspenders on top of viewportRect clamping)
    ...
    if (sidebarSvgRect) {
      if (isMobile) {
        ...
        // Full-height right-side band for the desktop column
